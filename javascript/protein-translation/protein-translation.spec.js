...
describe('ProteinTranslation', function () {
  ...
  it('Invalid codon throws error (wrong type)', function () {
    expect(
      function () {
        translate(42);
      }
  ...
  it('Invalid codon throws error: insufficient nucleotides', function () {
    expect(
      function () {
        translate('GG');
      }
    ).toThrow(new Error('Invalid codon: less than three nucleotides'));
  });
});
